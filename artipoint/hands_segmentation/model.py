...
class Denorm(object):
    ...
    def __call__(self, x):
        ...
        """
        denorm_fn = transforms.Normalize(
            mean=-self.mean / (self.std + 1e-8), std=1.0 / (self.std + 1e-8)
        )
        x_denorm = []
        for x_i in x:
...
class HandSegModel(pl.LightningModule):
    ...
    def __init__(self, pretrained=False, lr=1e-4, in_channels=3, *args, **kwargs):
        super().__init__()
        assert in_channels in [1, 3, 4]
        self.deeplab = self._get_deeplab(
            pretrained=pretrained, num_classes=2, in_channels=in_channels
        )
        ...
        if pretrained:
            if in_channels == 1:
                ...
                mean, std = np.array([0.485, 0.456, 0.406, 0.5]), np.array(
    ...
    def _get_deeplab(self, pretrained=False, num_classes=2, in_channels=3):
        ...
        if in_channels == 1:
            ...
            deeplab.backbone.conv1.weight = nn.Parameter(
                torch.cat(
                    [
                        weight.data,
